...
namespace cura
{
    ...
    GCodeExport::GCodeExport()
        : output_stream(&std::cout), currentPosition(0, 0, MM2INT(20)), layer_nr(0)
    {
    ...
    void GCodeExport::writeFiberCut()
    {
        const Settings &extruder_settings = Application::getInstance()
                                                .current_slice->scene.extruders[current_extruder]
                                                .settings;
        if (extruder_settings.get<bool>("machine_fiber_extruder"))
        {
            const std::string code = extruder_settings.get<std::string>("machine_fiber_cut_code");
    ...
    void GCodeExport::writeFiberPrime()
    {
        ...
        if (extruder_settings.get<bool>("machine_fiber_extruder"))
        {
            const std::string code = extruder_settings.get<std::string>("machine_fiber_prime_code");
    ...
    void GCodeExport::writeDryExtrusion(const Point &p, const Velocity &speed)
    {
        writeDryExtrusion(p.X, p.Y, current_layer_z + is_z_hopped, speed);
    }
                                        ...
                                        const Velocity &speed)
    {
        ...
        assert(speed < 400 && speed > 1); // normal F values occurring in UM2 gcode (this code
    ...
    void GCodeExport::writeRetraction(const RetractionConfig &config, bool force, bool extruder_switch)
    {
        ...
        { // handle retraction limitation
            ...
            if (!force && extruded_volume_at_previous_n_retractions.size() == config.retraction_count_max && current_extruded_volume < extruded_volume_at_previous_n_retractions.back() + config.retraction_extrusion_window * extr_attr.filament_area)
            {
                return;
            }
