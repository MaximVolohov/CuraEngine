...
namespace cura
{
    ...
    LayerPlan::LayerPlan(const SliceDataStorage &storage, LayerIndex layer_nr, coord_t z, coord_t layer_thickness, size_t start_extruder, const std::vector<FanSpeedLayerTimeSettings> &fan_speed_layer_time_settings_per_extruder, coord_t comb_boundary_offset, coord_t comb_move_inside_distance, coord_t travel_avoid_distance)
        : storage(storage), configs_storage(storage, layer_nr, layer_thickness), z(z), layer_nr(layer_nr), is_initial_layer(layer_nr == 0 - static_cast<LayerIndex>(Raft::getTotalExtraLayers())), is_raft_layer(layer_nr < 0 - static_cast<LayerIndex>(Raft::getFillerLayerCount())), layer_thickness(layer_thickness), has_prime_tower_planned_per_extruder(Application::getInstance().current_slice->scene.extruders.size(), false), current_mesh("NONMESH"), last_extruder_previous_layer(start_extruder), last_planned_extruder(&Application::getInstance().current_slice->scene.extruders[start_extruder]), first_travel_destination_is_inside(false) // set properly when addTravel is called for the first time (otherwise not set properly)
          ,
          comb_boundary_inside1(computeCombBoundaryInside(1)), comb_boundary_inside2(computeCombBoundaryInside(2)), comb_move_inside_distance(comb_move_inside_distance), fan_speed_layer_time_settings_per_extruder(fan_speed_layer_time_settings_per_extruder)
    {
    ...
    bool LayerPlan::setExtruder(const size_t extruder_nr, bool no_extruder_plan)
    {
    ...
    void LayerPlan::addPolygon(ConstPolygonRef polygon, int start_idx, const GCodePathConfig &config, WallOverlapComputation *wall_overlap_computation, coord_t wall_0_wipe_dist, bool spiralize, const Ratio &flow_ratio, bool always_retract, bool close)
    {
    ...
    */
    ...
    bool LayerPlan::writePathWithFiberCut(GCodeExport &gcode,
                                          const size_t extruder_plan_idx,
                                          const size_t path_idx,
                                          const coord_t layer_thickness)
    {
        ExtruderPlan &extruder_plan = extruder_plans[extruder_plan_idx];
        const ExtruderTrain &extruder = Application::getInstance()
                                            .current_slice->scene.extruders[extruder_plan.extruder_nr];
        const bool fiber_extruder = extruder.settings.get<bool>("machine_fiber_extruder");
        if (!fiber_extruder)
        {
            return false;
        }
        const coord_t fiber_cut_distance = MM2INT(extruder.settings.get<double>("machine_fiber_cut_distance"));
        ...
        coord_t fiber_min_dist_considered = 100;

        double extrude_speed = path.config->getSpeed() * extruder_plan.getExtrudeSpeedFactor(); // travel speed

        const coord_t fiber_cut_min_distance = MM2INT(
            extruder.settings.get<double>("machine_fiber_cut_min_distance"));

        std::vector<coord_t>
            accumulated_dist_per_point; // the first accumulated dist is that of the
                                        // last point! (that of the last point is
                                        // always zero...)
        accumulated_dist_per_point.push_back(0);
        ...
        unsigned int acc_dist_idx_gt_fiber_cut_dist = NO_INDEX;

        const Point *last = &path.points[path.points.size() - 1];

        for (unsigned int backward_point_idx = 1;
             backward_point_idx < path.points.size(); backward_point_idx++)
        {
            const Point &point = path.points[path.points.size() - 1 - backward_point_idx];
            const coord_t distance = vSize(point - *last);
            accumulated_dist += distance;
            accumulated_dist_per_point.push_back(accumulated_dist);
            if (acc_dist_idx_gt_fiber_cut_dist == NO_INDEX && accumulated_dist >= fiber_cut_distance)
            {
        ...
        assert(acc_dist_idx_gt_fiber_cut_dist < accumulated_dist_per_point.size());

        const size_t point_idx_before_start = path.points.size() - 1 - acc_dist_idx_gt_fiber_cut_dist;

        Point start;
        { // fiber cut point
            const coord_t residual_dist = fiber_cut_distance - accumulated_dist_per_point[acc_dist_idx_gt_fiber_cut_dist - 1];
            const Point &a = path.points[point_idx_before_start];
            const Point &b = path.points[point_idx_before_start + 1];
            start = b + normal(a - b, residual_dist);
        }
