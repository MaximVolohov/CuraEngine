...
namespace cura
{
    ...
    Polygons Polygons::calcConvexHull()
    {
        Polygon convex_hull;
        Polygons ret;
        std::vector<Point> source;
        Point leftmost_point = Point(std::numeric_limits<coord_t>::max(), std::numeric_limits<coord_t>::max());
        for (const ClipperLib::Path path : paths)
        {
        ...
        Point pointOnHull = leftmost_point;
        int i = 0;
        Point endpoint;
        do
        {
            convex_hull.add(pointOnHull);
            endpoint = source[0];
            for (size_t j = 0; j < source.size(); j++)
            {
                if (endpoint == pointOnHull || LinearAlg2D::pointIsLeftOfLine(source[j], convex_hull[i], endpoint) > 0)
                {
                    endpoint = source[j];
                }
            }
