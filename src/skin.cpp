 ...
 */
    ...
    int SkinInfillAreaComputation::getReferenceWallIdx(coord_t &preshrink) const
    {
        for (int wall_idx = wall_line_count; wall_idx > 0; wall_idx--)
        {
            ...
            coord_t next_wall_line_width = (next_wall_idx > 1) ? wall_line_width_x : (next_wall_idx == 0) ? 0
                                                                                                          : wall_line_width_0;
 ...
 */
    void SkinInfillAreaComputation::generateInfill(SliceLayerPart &part, const Polygons &skin)
    {
        ...
        {                                       // calculate offset_from_inner_wall
            ...
            if (fill_pattern == EFillMethod::CONCENTRIC && infill_line_distance > mesh.settings.get<coord_t>("infill_line_width") * 2)
            {
                if (mesh.settings.get<bool>("alternate_extra_perimeter") && layer_nr % 2 == 0)
