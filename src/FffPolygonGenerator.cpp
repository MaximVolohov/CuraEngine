        ...
        for (unsigned int meshIdx = 0; meshIdx < slicerList.size(); meshIdx++)
        {
            ...
            for (unsigned int layer_nr = 0; layer_nr < meshStorage.layers.size(); layer_nr++)
            {
                ...
                if (has_raft)
                {
                    ...
                    layer.printZ +=
                        Raft::getTotalThickness() + train.settings.get<coord_t>("raft_airgap") - train.settings.get<coord_t>("layer_0_z_overlap"); // shift all layers (except 0) down
...
#pragma omp for schedule(dynamic)
            ...
            {
                size_t reinforcement_start_layer = mesh.settings.get<size_t>("reinforcement_start_layer");
...
#pragma omp atomic
    ...
    void FffPolygonGenerator::processPerimeterGaps(SliceDataStorage &storage)
    {
        ...
        {
            constexpr int perimeter_gaps_extra_offset = 15; // extra offset so that the perimeter gaps aren't created everywhere due to rounding errors
            const bool fill_perimeter_gaps = mesh.settings.get<FillPerimeterGapMode>("fill_perimeter_gaps") != FillPerimeterGapMode::NOWHERE && !mesh_group_settings.get<bool>("magic_spiralize");
            ...
            for (LayerIndex layer_nr = 0; layer_nr < static_cast<LayerIndex>(mesh.layers.size()); layer_nr++)
            {
                ...
                bool fill_gaps_between_inner_wall_and_skin_or_infill =
                    mesh.settings.get<coord_t>("infill_line_distance") > 0 && mesh.settings.get<coord_t>("infill_overlap_mm") >= 0 && !(mesh.settings.get<EFillMethod>("infill_pattern") == EFillMethod::CONCENTRIC && (mesh.settings.get<bool>("alternate_extra_perimeter") || (layer_nr == 0 && train_wall_x.settings.get<Ratio>("initial_layer_line_width_factor") > 1.0)));
                SliceLayer &layer = mesh.layers[layer_nr];
                coord_t wall_line_width_0 = mesh.settings.get<coord_t>("wall_line_width_0");
 ...
 */
    void FffPolygonGenerator::processSkinsAndInfill(SliceMeshStorage &mesh, const LayerIndex layer_nr, bool process_infill, bool use_skin_on_layer)
    {
        if (mesh.settings.get<ESurfaceMode>("magic_mesh_surface_mode") == ESurfaceMode::SURFACE)
        {
            return;
        ...
        skin_infill_area_computation.generateSkinsAndInfill(use_skin_on_layer);
