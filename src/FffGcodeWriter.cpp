...
namespace cura
{
    ...
    FffGcodeWriter::FffGcodeWriter()
        : max_object_height(0), layer_plan_buffer(gcode)
    {
    ...
    void FffGcodeWriter::setInfillAndSkinAngles(SliceMeshStorage &mesh)
    {
        ...
        {
            mesh.fiber_infill_angles = mesh.settings.get<std::vector<AngleDegrees>>("fiber_infill_angles");
            if (mesh.fiber_infill_angles.size() == 0)
            {
                mesh.fiber_infill_angles.push_back(45); // generally all infill patterns use 45 degrees
                // lines and zig zag patterns default to also using 135 degrees
                mesh.fiber_infill_angles.push_back(135);
            }
        }
    ...
    LayerPlan &FffGcodeWriter::processLayer(const SliceDataStorage &storage, LayerIndex layer_nr, const size_t total_layers) const
    {
        ...
        else
        {
            ...
            {
                if (layer_nr >= static_cast<int>(mesh.layers.size()) || mesh.settings.get<bool>("support_mesh") || mesh.settings.get<bool>("anti_overhang_mesh") || mesh.settings.get<bool>("cutting_mesh") || mesh.settings.get<bool>("infill_mesh"))
                {
                    continue;
                }
...
#pragma omp critical
        ...
        const std::vector<size_t> &extruder_order =
            (layer_nr < 0) ? extruder_order_per_layer_negative_layers[extruder_order_per_layer_negative_layers.size() + layer_nr]
    ...
    std::vector<size_t> FffGcodeWriter::getUsedExtrudersOnLayerExcludingStartingExtruder(const SliceDataStorage &storage, const size_t start_extruder, const LayerIndex &layer_nr) const
    {
        ...
        if ((mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::RAFT && layer_nr == -static_cast<LayerIndex>(Raft::getTotalExtraLayers())) || (mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") != EPlatformAdhesion::RAFT && layer_nr == 0))
        {
    ...
    {
        ExtruderTrain &infill_extruder = Application::getInstance()
                                             .current_slice->scene
                                             .extruders[mesh.settings.get<ExtruderTrain &>("infill_extruder_nr")
                                                            .extruder_nr];
        bool fiber_extruder = infill_extruder.settings.get<bool>("machine_fiber_extruder");

        if (!fiber_extruder && extruder_nr != mesh.settings.get<ExtruderTrain &>("infill_extruder_nr").extruder_nr)
        {
            return false;
        }
    ...
    bool FffGcodeWriter::processSingleLayerInfill(const SliceDataStorage &storage, LayerPlan &gcode_layer, const SliceMeshStorage &mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs &mesh_config, const SliceLayerPart &part) const
    {
        ...
        if (extruder_nr != infill_extruder.extruder_nr)
        {
            return false;
        }
        const coord_t infill_line_distance = mesh.settings.get<coord_t>("infill_line_distance");
        ...
        if (infill_lines.size() > 0 || infill_polygons.size() > 0)
        {
            ...
            setExtruder_addPrime(storage, gcode_layer, extruder_nr);
            gcode_layer.setIsInside(true); // going to print stuff inside print object
            if (!printable_infill_polygons.empty())
            {
                constexpr bool force_comb_retract = false;
                gcode_layer.addTravel(printable_infill_polygons[0][0], force_comb_retract);
                gcode_layer.addPolygonsByOptimizer(printable_infill_polygons, mesh_config.infill_config[0], nullptr, ZSeamConfig(), 0LL, false, 1.0_r, false, false);
            }
            const bool enable_travel_optimization = mesh.settings.get<bool>("infill_enable_travel_optimization");
            ...
            {
                gcode_layer.addLinesByOptimizer(printable_infill_lines, mesh_config.infill_config[0], SpaceFillType::Lines, enable_travel_optimization, mesh.settings.get<coord_t>("infill_wipe_dist"));
            }
            else
            {
    ...
    bool FffGcodeWriter::processFiberInfill(const SliceDataStorage &storage, LayerPlan &gcode_layer, const SliceMeshStorage &mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs &mesh_config, const SliceLayerPart &part) const
    {
        ExtruderTrain &fiber_infill_extruder = Application::getInstance()
                                                   .current_slice->scene
                                                   .extruders[mesh.settings.get<ExtruderTrain &>("fiber_infill_extruder_nr")
                                                                  .extruder_nr];
        ExtruderTrain &skin_extruder = Application::getInstance()
                                           .current_slice->scene
                                           .extruders[mesh.settings.get<ExtruderTrain &>("top_bottom_extruder_nr")
                                                          .extruder_nr];
        bool fiber_extruder = fiber_infill_extruder.settings.get<bool>("machine_fiber_extruder");
        ...
        {
            processPerimeterGaps(storage, gcode_layer, mesh, extruder_nr, output_gaps, mesh_config.skin_config, added_something);
            return added_something;
        }
        ...
        if (infill_lines.size() > 0 || infill_polygons.size() > 0)
        {
            ...
            if (pattern == EFillMethod::GRID || pattern == EFillMethod::LINES || pattern == EFillMethod::TRIANGLES || pattern == EFillMethod::CUBIC || pattern == EFillMethod::TETRAHEDRAL || pattern == EFillMethod::QUARTER_CUBIC || pattern == EFillMethod::CUBICSUBDIV)
            {
                gcode_layer.addLinesByOptimizer(printable_infill_lines, mesh_config.fiber_infill_config, SpaceFillType::Lines, enable_travel_optimization, mesh.settings.get<coord_t>("infill_wipe_dist"));
            }
            else
            {
                gcode_layer.addLinesByOptimizer(printable_infill_lines, mesh_config.fiber_infill_config, (pattern == EFillMethod::ZIG_ZAG) ? SpaceFillType::PolyLines : SpaceFillType::Lines, enable_travel_optimization);
            }
        }
    ...
    void
    FffGcodeWriter::processSpiralizedWall(const SliceDataStorage &storage, LayerPlan &gcode_layer, const PathConfigStorage::MeshPathConfigs &mesh_config, const SliceLayerPart &part) const
    {
    ...
    bool FffGcodeWriter::processInsets(const SliceDataStorage &storage, LayerPlan &gcode_layer, const SliceMeshStorage &mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs &mesh_config, const SliceLayerPart &part) const
    {
        ...
        if (mesh.settings.get<size_t>("wall_line_count") > 0)
        {
            ...
            {
                const bool outer_inset_first = mesh.settings.get<bool>("outer_inset_first") || (gcode_layer.getLayerNr() == 0 && mesh.settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::BRIM);
    ...
    bool FffGcodeWriter::processSkinAndPerimeterGaps(const SliceDataStorage &storage, LayerPlan &gcode_layer, const SliceMeshStorage &mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs &mesh_config, const SliceLayerPart &part) const
    {
        ...
        if (extruder_nr != top_bottom_extruder_nr && extruder_nr != wall_0_extruder_nr && (extruder_nr != roofing_extruder_nr || mesh.settings.get<size_t>("roofing_layer_count") <= 0))
        {
            return false;
        }
        bool added_something = false;

        const bool fill_perimeter_gaps = mesh.settings.get<FillPerimeterGapMode>("fill_perimeter_gaps") != FillPerimeterGapMode::NOWHERE && !Application::getInstance().current_slice->scene.current_mesh_group->settings.get<bool>("magic_spiralize") && extruder_nr == wall_0_extruder_nr;
    ...
    void FffGcodeWriter::processRoofing(const SliceDataStorage &storage, LayerPlan &gcode_layer, const SliceMeshStorage &mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs &mesh_config, const SkinPart &skin_part, Polygons &concentric_perimeter_gaps, bool &added_something) const
    {
        ...
        const bool fill_perimeter_gaps =
            mesh.settings.get<FillPerimeterGapMode>("fill_perimeter_gaps") != FillPerimeterGapMode::NOWHERE && !Application::getInstance().current_slice->scene.current_mesh_group->settings.get<bool>("magic_spiralize");
    ...
    void FffGcodeWriter::processTopBottom(const SliceDataStorage &storage, LayerPlan &gcode_layer, const SliceMeshStorage &mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs &mesh_config, const SkinPart &skin_part, Polygons &concentric_perimeter_gaps, bool &added_something) const
    {
        ...
        const bool generate_perimeter_gaps =
            mesh.settings.get<FillPerimeterGapMode>("fill_perimeter_gaps") != FillPerimeterGapMode::NOWHERE && !mesh_group_settings.get<bool>("magic_spiralize");
    ...
    bool FffGcodeWriter::processSupportInfill(const SliceDataStorage &storage, LayerPlan &gcode_layer) const
    {
        ...
        for (int part_idx : island_order_optimizer.polyOrder)
        {
            ...
            if (default_support_line_distance <= 0 || part.infill_area_per_combine_per_density.empty())
            {
                continue;
            }
