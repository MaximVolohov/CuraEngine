...
namespace cura
{
   ...
   class Infill
   {
      ...
      coord_t minimum_infill_line_length;
   ...
   public:
     ...
     */
      Infill(EFillMethod pattern, bool zig_zaggify, bool connect_polygons, const Polygons &in_outline, coord_t outline_offset, coord_t infill_line_width, coord_t line_distance, coord_t infill_overlap, size_t infill_multiplier, AngleDegrees fill_angle, coord_t z, coord_t shift, size_t wall_line_count = 0, const Point &infill_origin = Point(), Polygons *perimeter_gaps = nullptr, bool connected_zigzags = false, bool use_endpieces = false, bool skip_some_zags = false, size_t zag_skip_count = 0, coord_t pocket_size = 0, coord_t minimum_zag_line_length = DEFAULT_MINIMUM_LINE_LENGTH_THRESHOLD, coord_t minimum_infill_line_length = 0)
          : pattern(pattern), zig_zaggify(zig_zaggify), connect_polygons(connect_polygons), in_outline(in_outline), outline_offset(outline_offset), infill_line_width(infill_line_width), line_distance(line_distance), infill_overlap(infill_overlap), infill_multiplier(infill_multiplier), fill_angle(fill_angle), z(z), shift(shift), wall_line_count(wall_line_count), infill_origin(infill_origin), perimeter_gaps(perimeter_gaps), connected_zigzags(connected_zigzags), use_endpieces(use_endpieces), skip_some_zags(skip_some_zags), zag_skip_count(zag_skip_count), pocket_size(pocket_size), minimum_zag_line_length(minimum_zag_line_length), minimum_infill_line_length(minimum_infill_line_length)
      {
      }
     ...
     */
      void generate(Polygons &result_polygons, Polygons &result_lines, Polygons &result_gaps, const SierpinskiFillProvider *cross_fill_provider = nullptr, const SliceMeshStorage *mesh = nullptr);
   ...
   private:
      ...
      struct InfillLineSegment
      {
         ...
         */
         InfillLineSegment(const Point start, const size_t start_segment, const size_t start_polygon, const Point end, const size_t end_segment, const size_t end_polygon)
             : start(start), start_segment(start_segment), start_polygon(start_polygon), end(end), end_segment(end_segment), end_polygon(end_polygon), previous(nullptr), next(nullptr){};
     ...
     */
      ...
      void generateFiberInfill(Polygons &result, const coord_t line_distance, const double &infill_rotation);
